...
using DSMRReader.Net.Constants;
using DSMRReader.Net.Models;
using RestEase;

namespace DSMRReader.Net.Client;

/// <summary>
/// Summary: https://dsmr.home.decramy.net/docs/api/redoc
...
public interface IDSMRReaderApi
{
    ...
    Task<PagedResult<Reading>> GetDataLoggerReadingsAsync([Query] int? limit = null, [Query] int? offset = null, [Query] string? ordering = null, [Query] string? timestamp = null, [Query(Name = "timestamp__gte")] string? timestampGte = null, [Query(Name = "timestamp__lte")] string? timestampLte = null);
    ...
    Task<Reading> CreateReadingAsync([Body] Reading content);
    ...
    Task<MeterStatistics> GetMeterStatisticsAsync([Query] string? ordering = null);
    ...
    /// Notes- These are the contracts manually entered in DSMR-reader's admin interface.
    /// Can be used for manual calculations.> *E.g. fetching any contracts active today, then fetch all day statistics filtered by the contract's start/end,finally summing up the total consumption for that contract.
    /// Similar to DSMR-reader's GUI regarding contact totals.
    ///
    /// ### Query parameters- *Only mandatory when explicitly marked with the **required** label.
    /// Can be omitted otherwise.
    /// - ``limit`` / ``offset``: Optional pagination. Probably not needed unless you have *a lot* of contracts.
    /// - ``ordering``: Order by either ``start`` (ASC), ``-start`` (DESC), ``end`` (ASC) or ``-end`` (DESC).- ``start__gte`` / ``start__lte`` / ``end__gte`` / ``end__lte``: Can be used for generic filtering the results returned by contracts' start/end with the given date as placeholder `X` below.
    ///   Note the ``Y-m-d`` format for `X`.
    ///
    /// ### Changes- This endpoint was added in DSMR-reader v5.3
    ///
    /// ### Request samples
    ...
    Task<PagedResult<EnergySupplierPrice>> GetEnergySupplierPricesAsync([Query] int? limit = null, [Query] int? offset = null, [Query] string? ordering = null, [Query(Name = "start__gte")] string? startGte = null, [Query(Name = "start__lte")] string? startLte = null, [Query(Name = "end__gte")] string? endGte = null, [Query(Name = "end__lte")] string? endLte = null);
    ...
    Task<PagedResult<ElectricityConsumption>> GetElectricityConsumptionsAsync([Query] int? limit = null, [Query] int? offset = null, [Query] string? ordering = null, [Query(Name = "read_at")] string? readAt = null, [Query(Name = "read_at__gte")] string? readAtGte = null, [Query(Name = "read_at__lte")] string? readAtLte = null);
    ...
    Task<PagedResult<QuarterHourPeakElectricityConsumption>> GetQuarterHourPeakElectricityConsumptionsAsync([Query] int? limit = null, [Query] int? offset = null, [Query] string? ordering = null, [Query(Name = "read_at_start__gte")] string? readAtStartGte = null, [Query(Name = "read_at_start__lte")] string? readAtStartLte = null, [Query(Name = "average_delivered__gte")] string? averageDeliveredGte = null, [Query(Name = "average_delivered__lte")] string? averageDeliveredLte = null);
    ...
    Task<ElectricityLive> GetElectricityConsumptionLiveAsync();
    ...
    Task<PagedResult<GasConsumption>> GetGasConsumptionsAsync([Query] int? limit = null, [Query] int? offset = null, [Query] string? ordering = null, [Query(Name = "read_at")] string? readAt = null, [Query(Name = "read_at__gte")] string? readAtGte = null, [Query(Name = "read_at__lte")] string? readAtLte = null);
    ...
    Task<GasLive> GetGasConsumptionLiveAsync();
    ...
    Task<ApplicationVersion> GetApplicationVersionAsync();
    ...
    Task<ApplicationMonitoring> GetApplicationMonitoringAsync();
}